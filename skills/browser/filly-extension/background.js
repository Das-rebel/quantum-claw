/**
 * FillY — Background Service Worker
 * 
 * Handles: side panel, badge updates, message routing, profile storage
 */

const DEBUG = true;
const log = (...args) => { if (DEBUG) console.log('[FillY BG]', ...args); };
...
async function updateBadge(tabId, url) {
  ...
  if (ats) {
    ...
    await chrome.action.setTitle({ tabId, title: `FillY — ${ats} detected!` });
  } else {
    await chrome.action.setBadgeText({ tabId, text: '' });
    await chrome.action.setTitle({ tabId, title: 'FillY' });
  }
}
  ...
  switch (message.type) {
    ...
    case 'GET_PROFILE':
      chrome.storage.local.get(['profile'], (result) => {
        sendResponse({ profile: result.filly_profile || null });
      });
      return true;

    case 'SAVE_PROFILE':
      chrome.storage.local.set({ profile: message.filly_profile }, () => {
    ...
    case 'AUTOFILL':
      chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
        ...
        if (message.filly_profile) {
          chrome.tabs.sendMessage(tabs[0].id, {
            type: 'AUTOFILL',
            profile: message.filly_profile,
...
log('FillY background service worker loaded');
